...
PIXEL_DIFF_THRESH = 30    # How much a single pixel must change (0-255) to be "different"
CHANGED_PIXELS_MIN = 50   # How many "different" pixels constitute a moved piece
...
def get_background_vision(model, frame, last_valid, board, classifier):
    ...
    # 3. Change Detection (PIXEL COUNT METHOD)
    ...
    for sq in chess.SQUARES:
        ...
        # Threshold: Only count pixels that changed SIGNIFICANTLY (e.g., > 30 value)
        _, thresh = cv2.threshold(diff, PIXEL_DIFF_THRESH, 255, cv2.THRESH_BINARY)
        
        # Count changed pixels
        changed_pixels = cv2.countNonZero(thresh)
        
        has_changed = changed_pixels > CHANGED_PIXELS_MIN
        
        if has_changed:
            changed_squares.add(sq)
            # If visual change -> Trust Vision
            if (sq in yolo_occ) or (sq in class_occ):
                final_occ.add(sq)
        else:
            # If no change -> Trust History
...
def find_best_matching_move(board, visual_occupancy):
    """
    Finds best move, prioritizing Valid Moves over 'No Move' on ties.
    """
    best_move = None
    min_error = float('inf')
    ...
    # Only scan moves involving squares that look WRONG
    ...
    # TIE BREAKER: If the move fits perfectly (Error 0) or is better than doing nothing
    if best_move:
        if min_error < current_error:
            return best_move
        elif min_error == current_error and min_error <= 1:
            return best_move
...
def draw_hud(image, board, changed_squares):
    ...
    for r in range(8):
        for c in range(8):
            ...
            # Highlight Changed Squares (Blue Box) - DEBUGGING
            color = (0,255,255)
            if sq in changed_squares:
                cv2.rectangle(overlay, (x, y), (x+SQUARE_SIZE, y+SQUARE_SIZE), (255,0,0), 2)
                color = (255,0,0) # Blue text for active squares
            else:
                cv2.rectangle(overlay, (x, y), (x+SQUARE_SIZE, y+SQUARE_SIZE), (0,255,255), 1)
                
            cv2.putText(overlay, chess.square_name(sq).upper(), (x+5, y+15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
            
            # LOGIC STATE (Green/Red)
            if board.piece_at(sq):
...
def process_video():
    ...
    while True:
        ...
        if check_hand_motion(warped, prev_frame):
            hand_timer += 1
            if hand_timer < 30: 
                cv2.putText(warped, "HAND", (50, 300), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,0,255), 4)
